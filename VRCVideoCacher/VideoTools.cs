namespace VRCVideoCacher
{
    public class VideoTools
    {
        ...
        public static async Task Prefetch(string videoUrl)
        {
            ...
            {
                Log.Information("Prefetching M3U8 stream URL");

                int statusCode = 0;
                int requestLimit = 7;
                int wait = 1500;

                for (int i = 0; i < requestLimit && statusCode != 200; i++)
                {
                    ...
                    if (statusCode >= 400)
                    {
                        Log.Warning("Prefetching M3U8 stream returned status code {status}, retrying... ({attempt}/{limit})", statusCode, i + 1, requestLimit);
                ...
                if (statusCode != 200)
                {
                    Log.Error("Prefetching M3U8 stream failed after {limit} attempts, status code {status}. Video may not play.", requestLimit, statusCode);
                }
            }
        }
